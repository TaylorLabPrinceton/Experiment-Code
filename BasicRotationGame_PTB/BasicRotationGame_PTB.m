...
% Its recommended to add 15 msecs latency on Windows, to protect against shoddy drivers:
...
[miss_sound, freq2] = audioread('incorrect.wav'); % too slow voice
...
[neutral_sound, freq5] = audioread('neutral.wav'); % too fast/slow
...
elseif (exist('/Acer')>0);
    ...
    disp('Warning: unknown monitor. Assuming Planar monitor settings. If these are wrong will screw up distances and screen mappings.');
    c.width_mm = 480; % Planar touch screen
    c.height_mm = 270; % Planar touch screen 
end
...
c.start_x = c.screenWidth/2; % middle of screen x
c.start_y = c.screenHeight/2; % middle of screen y
c.tab_center_x = c.tab_x_max/2;
c.tab_center_y = c.tab_y_max/2;
c.cursor_radius = 6; % radius of cursor
c.target_radius = 10; % larger than cursor
...
orange = [255,100,0];
target_color = green; % define target color
cursor_color = white; % cursor color

%% Constants for trial parameters
% These are the fixed task delays
c.target_wait = 1; % number of seconds until target appears
c.feedback_time = 1; % seconds for fb
c.reach_limit = .60; % max seconds allowed for reach
c.reach_min = .40; % min seconds allowed


%% Define instruction screens
IntroText = 'The goal of the game is to get your cursor on the green target';
EPText = 'You will now only see feedback at the end of your reach';
AimText = 'Please report aim';
WashText = 'You will no longer receive feedback, \n move directly to the target';
...
targetfile.target_dist=trial_file(:,strcmp('target_dist',targetheaders));
...
large_array_size = 2e5; % at the moment anything larger makes game slow
...
fb_hand_x = nan(num_Trials,large_array_size);
fb_hand_y = nan(num_Trials,large_array_size);
...
timers = struct('hold_start',nan(num_Trials,1),'hold_end',nan(num_Trials,1),'aim_onset',nan(num_Trials,1),'aim_report',nan(num_Trials,1),'go',nan(num_Trials,1),'move_start',nan(num_Trials,1),'move_end',nan(num_Trials,1),'FB_onset',nan(num_Trials,1));
%% Timers
t_hold_start = NaN;
t_hold_end = NaN;
t_aim_onset = NaN;
...
%start_time = start_time_tablet/1000; %sync timing
tabdata = pkt;
...
%         
        ...
        if ~isempty(tabdata)
            ...
            if abs(targetfile.rotation(trialnum)) > 0
                ...
                rotated = [axisStateX-c.start_x axisStateY-c.start_y]*rot_matrix;
        ...
        if e_hold_start == 1
            if NewTabData
                ...
                % holding at start?
                if dist_max > c.start_area % they left the start position
                    e_find_start = 1;
                    e_hold_start = 0;
                end
            end
            ...
            if hold_count-t_hold_start >= c.target_wait;
                ...
                Screen(window_ptr, 'FillOval', red, target);
        ...
        if e_go_signal == 1
           event_idx(trialnum,count) = 5;
            ...
            if firstloop == true
                ...
                [~, rt_start_t, ~, ~, ~]=Screen('Flip',window_ptr);%wait for sync
                rt_start_t=rt_start_t-start_time;
                t_go=rt_start_t;
                ...
                if dist_last > c.start_area
                    ...
                    RT = t_move_start - rt_start_t; % log subject's RT
                end
                firstloop = false;
            else
                Screen(window_ptr, 'FillOval', white, [c.start_x-c.start_area,c.start_y-c.start_area,c.start_x+c.start_area,c.start_y+c.start_area]);
        ...
        if e_reaching == 1
            if NewTabData
                ...
                if dist_max >= ring_radius(trialnum) % reach complete
                    ...
                    % Sounds
                    % "too slow" sound
                    if movement_time >= c.reach_limit % over 600 ms
                        hit = 0;
                        e_too_slow = 1;
                    end
                    % with a low-latency sound card we could beat the
                    % visual FB. Move later if so.
                    if targetfile.endpoint_fb(trialnum) == 0; % no FB trials
                        fb_color = green;
                    ...
                    % Target
                    Screen(window_ptr, 'FillOval', fb_color, target);
                    ...
                    t_FB_onset=t_FB_onset-start_time;
                    if e_too_slow
        ...
        if e_data == 1
            ...
            if targetfile.instruction(trialnum) == 1 % use for short text
                ...
                DrawFormattedText(window_ptr,EPText,'center','center',white); 
                Screen('Flip',window_ptr, [], 1);
                PsychPortAudio('Start', pa_start, 1, 0, 1); % wake up experimenter!
                KbWait(-1);
        ...
        if count > large_array_size && trialtoolong==false
            msg=strcat('Trial',{' '},num2str(trialnum),' too long, losing data');
            disp(msg);
